# Enlaces para el módulo de Programación

## Curso 2025/2026

### Herencia 

**10/11/2025** (y más) [Varios ejemplos con condicionales](https://github.com/ehguille/condicionales)

**07/11/2025** [Ejemplo con figuras geométricas para trabajar herencia, abstracción, atributos estáticos y orientación a objetos en general](https://github.com/ehguille/geometria).

**06/11/2025** [Herencia e interfaces con electrodomésticos](https://github.com/ehguille/electrodomesticos).

**01/11/2025** [Ejemplos de hencia con vehículos](https://github.com/ehguille/vehiculos).

**27/10/2025** [Herencia en un zoológico](https://github.com/ehguille/zoologico).

### Orientación a objetos básica

**22/09/2025**  [Ejemplo básico de un simulador de conducción](https://github.com/ehguille/simulador-conduccion).

**22/09/2025** [Ejemplos iniciales de programación en Java](https://github.com/ehguille/prog.intro).
